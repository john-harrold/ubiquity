...
Or these can be written as equilibrium equations:

~~~
A + B  <=konR:koffR=>  C       
~~~

To write this as differential equations, the following would be used:
...
The events contain the magnitude of the bolus at a given time. If you have multiple compartments receiving a bolus, the times must include all times in which a bolus may be applied to the system. If a state does not receive a bolus on a particular time, it’s magnitude at that time is 0.
...
By default sets will interpreted by evaulating every possible permutation as shown above. However, it may be desireable to pair or align sets. Take for example the transit compartments implemented by Lobo and Balthasar to delay the onset of drug effect on cancer cells [LB]. The traisnt compartment are implemented using a series of differential equations:
...
```
Next we write the ``<ODE:?>?`` statment in terms of these sets, but we use the ``SIMINT_SET_ALIGN`` function:

```
SIMINT_SET_ALIGN[TRIN;TROUT][<ODE:{TROUT}> 1.0/tau*({TRIN}-{TROUT})]
```

The first argument is the name sets to align separated by a simicolon ``;`` and the second argment is the system file descriptor written in terms of these sets. This will be expanded internall into:
...
These functions take two bracketed arguments. The first argument is set name and the second argument is the mathematical relationship to be expanded. For example, consider a system that has been parameterized for several species. The variable ``EN_Mouse`` is 1 if the species is mouse and 0 otherwise. Similarly for ``EN_Human``, ``EN_Monkey``, etc. We have also defined the body weights as system parameters: ``BW_Mouse`` for the mouse, ``BW_Human`` for human, etc. Now we want to assign ``BW`` to the value of the currently seleted species (where ``EN`` for that species is 1). First we would define the species parameter set:
...
Sets are evaulated in the following order:

* First set functions (``SIMINT_SET_SUM`` and ``SIMINT_SET_PRODUCT``) are evaulated.
* Next aligned sets are applied (``SIMINT_SET_ALIGN``)
* Lastly, remaining sets are evaulated for every permutation 
...
These example system files can be found in the examples directory of the stand-alone analysis template ([ubiquity_template.zip](http://template.ubiquity.tools)). Most can also be loaded form the R package (see the help for system_new).


* ``system-adapt.txt`` - Parent/metabolite structural model taken from the ADAPT5 Users Manual [ADAPT]. This sytem is used in the estmation examples of the ubiquity workshop.
* ``system-glp_study.txt`` - PK model paramterized for humans and NHPs, used as an example for GLP tox study design.
...
* ``system-one_cmt_macro.txt``  - One compartment model parameterizedf or macro constants (clearances and volumes).
* ``system-one_cmt_micro.txt`` - One compartment model parameterizedf or micro constants (rates).
...
* ``system-sets.txt`` - Example of how to parameterize systems with multiple parameter sets.
...
* ``system-two_cmt_macro.txt`` - Two compartment model parameterizedf or macro constants (clearances and volumes)
* ``system-two_cmt_micro.txt`` - Two compartment model parameterizedf or micro constants (rates)

## References

* [ADAPT][https://bmsr.usc.edu/files/2013/02/ADAPT5-User-Guide.pdf](https://bmsr.usc.edu/files/2013/02/ADAPT5-User-Guide.pdf)
