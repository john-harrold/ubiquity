...
This example is similar to the last except here we are performing population simulations (20 for an example). The same function (``simulate_subjects``) is used but setting the ``'titrate'`` option to ``TRUE`` tells that function to use the rule-based dosing. 
