...
```{r warning=FALSE, message=FALSE, echo=FALSE, error=FALSE, fig.width=8, fig.height=3.5}
grid.arrange(p_AUC, p_AUMC, ncol=2)
```
...
rhandsontable(cfg$data$PKDATA$values, width=500, height=200)
```
...
* ``output/pk_single_dose-nca_summary.csv``  Summary level output from the analysis
...
```{r warning=FALSE, message=FALSE, echo=FALSE, error=FALSE, fig.width=8, fig.height=4}
nca_summary = read.csv(file.path("output", "pk_single_dose-nca_summary.csv"))
...
* ``Vss_obs`` Steady-state volume of distribution ?
(``vss.obs``)
* ``Vss_pred`` Steady-state volume of distribution ?
  (``vss.pred``
* ``C0`` Back-extrapolated initial concentration
* ``CL_obs`` Clearance ? (``cl.obs``)
* ``CL_pred`` Clearance ? (``cl.pred``)
* ``AUClast`` Area under the curve ? (``auclast``)
* ``AUCinf_pred`` Area under the curve ? (``aucinf.pred``)
* ``AUCinf_obs`` Area under the curve ? (``aucinf.obs``)
...
![Reporting for single dose NCA](nca_reporting-sd.png){width=100%} 
...
```{r echo=FALSE, fig.align="center"}
rhandsontable(cfg$data$PKDATA$values, width=800, height=200)
```
...
```{r warning=FALSE, message=FALSE, echo=FALSE, error=FALSE, fig.width=8, fig.height=4}
nca_summary = read.csv(file.path("output", "pk_multiple_dose-nca_summary.csv"))
rhandsontable(nca_summary,  width=500, height=200)
```
...
![Reporting for multiple dose NCA](nca_reporting-md.png){width=100%} 
...
nca_summary = read.csv(file.path("output", "pk_sparse-nca_summary.csv"))
rhandsontable(nca_summary,  height=150, width=500)
```

To confirm what was done and identify any outliers that may be causing problem you can use the report output. Again the grey markers show the data, the green dashed line will show the value used for NCA, and the orange will show the C0 estimate. 

![Reporting for sparse sampling NCA](nca_reporting-sparse.png){width=100%} 
