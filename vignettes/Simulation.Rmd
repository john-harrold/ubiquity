...
The workshop (http://workshop.ubiquity.grok.tv) provides several examples of how to perform simulations in ubiquity. To make a copy of these scripts and other supporting files in the current working directory run the following:
...
This should create the folloing scripts:
...
These rely on a PK model of mAbs in humans ([Davda etal. mAbs, 6(4), 1094-1102](http://doi.org/10.4161/mabs.29095)).  The contents of the system file for this model can be seen at the bottom. The first step in any analysis (simulation, estimation, etc) is building the system file creating the ubiquity model object (``cfg``). 
