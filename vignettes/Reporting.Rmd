...
fr = system_rpt_template_details(cfg_pptx)
trim_idx = min(c(18, length(fr$txt)))
cat(paste(fr$txt[1:trim_idx], collapse="\n"))
```
...
```{r message=FALSE, warning=FALSE, eval=TRUE, echo=FALSE, message=FALSE}
fr = system_rpt_template_details(cfg_docx)
cat(paste(fr$txt, collapse="\n"))
```
...
Internally `ubiquity` uses the `onbrand` package for customizable templates. To create a template for your organization you should use the `onbrand` vignette for [Custom Templates]( http://onbrand.ubiquity.tools/articles/Custom_Office_Templates.html).  This will instruct you to create a yaml mapping file for your templates. You can use the built in organizational template as a starting point for that:
...
tmplt_deets = system_rpt_template_details(cfg_pptx)
tmplt_deets$ft
```
...
```{r message=FALSE, warning=FALSE, eval=TRUE, echo=FALSE}
tmplt_deets = system_rpt_template_details(cfg_docx)
tmplt_deets$ft
```
...
|`system_rpt_doc_content()`             |    `onbrand::report_add_doc_content()`               | 
