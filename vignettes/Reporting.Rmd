...
Each element in a master is identified with an index number. These numbers are asigned as the master slide is built, so it can be difficult to identify what these numbers are through PowerPoint. If you named your template ``mytemplate.pptx``, then you can have ubiquity produce a slide deck with the masters annotated so you can identify these indices:
...
This should create a file called ``layout.pptx`` that has the ``layout`` name of each slide master in the title and the ``index`` of each body element identified. These slides should look something like:
