...
* ``"table"`` data frame with with names used as column headers
...
cfg = system_report_slide_content(cfg, 
           ...
           content      = system.file("vignettes", "report_image.png", package="ubiquity"))
```

The benefit of using ``ggplot`` is that it will automatically size to the placeholder you put the image in. To use an image file the image file needs to have the same aspect ratio as the placeholder you want to put it in. 
