...
To demonstrate this interface we can begin with the PK model of mAbs in humans ([Davda etal. mAbs, 6(4), 1094-1102](http://doi.org/10.4161/mabs.29095)):
...
After building any system you can then create the ShinyApp template:
...
![Bolus Dosing](ShinyApp-ss-Bolus.png){width=30%}  
...
![Continuous Infusion](ShinyApp-ss-Infusion.png){width=30%}
...
![Population simulation options](ShinyApp-ss-IIV.png){width=30%}
...
![Population timecourse](ShinyApp-ss-Pop_timecourse.png){width=30%}

##Saving model results   

![Saving an analysis](ShinyApp-ss-save.png){width=30%}
...
In certain instances you may not want to allow the user access to everythign that is saved by default. If you edit the ``ubiquity_app.R`` you can alter what is being pushed to the user by setting the relevant fields to ``FALSE``.
...
You can create this image in your favorite drawing program. Inkscape is a free vector drawing program available at [inkscape.org](http://inkscape.org). If you run the following:
...
Next run the test script and modify the report name accordingly:
...
This shoudl generate your report in HTML format. You can insert debugging commands in the report and test it while you add information. Once you have it working, you can enable the report as described above and it shoudl appear as a tab viewable by the user.  
...
To deploy the ShinyApp on a shiny server you need to indicate this by modifying ``ubiquity_app.R`` and chainging the deploying variable to ``TRUE``:
...
Then the ShinyApp needs to be initialized on the server by running the following **as the shiny user** on the server:

```{r, eval=FALSE}
R -e "source('model_gui.r')"
```


If the App fails to run on the Shiny Server two options should be checked:
* Make sure all of the correct packages have been installed and are avialble to the user the Shiny Server is running under.
